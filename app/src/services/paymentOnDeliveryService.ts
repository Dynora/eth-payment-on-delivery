...
export class PaymentOnDeliveryService {
    ...
    async settleDelivery(delivery: Delivery, signature: string): Promise<string> {
        ...
        const result = await instance.settleDelivery(delivery.deliveryId, signature, {
            from: account,
            gasLimit: 4700000
        });
    ...
    async getDelivery(deliveryId: string): Promise<Delivery> {
        ...
        const created = await instance.getDeliveryCreated(deliveryId, {from: account});
        ...
        if (!active) {
          refunded = await instance.getDeliveryRefunded(deliveryId, {from: account});
          if (refunded) {
            refunded = new Date(refunded.toNumber() * 1000);
          }
          delivered = await instance.getDeliveryDelivered(deliveryId, {from: account});
          if (delivered) {
            delivered = new Date(delivered.toNumber() * 1000);
          }
        }
        ...
        return {
            ...
            createdTimestamp: new Date(created.toNumber() * 1000),
            refundedTimestamp: refunded,
